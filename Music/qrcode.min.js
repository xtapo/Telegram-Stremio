 ...
 * Local standalone bundle for XTAPO Music
 */
(function(window, document) {
        ...
        make: function() {
            if (this.typeNumber < 1) {
                ...
                for (typeNumber = 1; typeNumber < 40; typeNumber++) {
                    ...
                    if (buffer.getLength() <= totalDataCount * 8) break;
                }
                this.typeNumber = typeNumber;
            }
    ...
    QRCodeModel.createData = function(typeNumber, errorCorrectLevel, dataList) {
        ...
        var totalDataCount = 0;
        for (var i = 0; i < rsBlocks.length; i++) totalDataCount += rsBlocks[i].dataCount;
        if (buffer.getLength() > totalDataCount * 8) throw new Error("code length overflow. (" + buffer.getLength() + ">" + totalDataCount * 8 + ")");
        if (buffer.getLength() + 4 <= totalDataCount * 8) buffer.put(0, 4);
        while (buffer.getLength() % 8 != 0) buffer.putBit(false);
        while (true) {
            if (buffer.getLength() >= totalDataCount * 8) break;
            buffer.put(0xEC, 8);
            if (buffer.getLength() >= totalDataCount * 8) break;
    ...
    function QRCode(targetEl, options) {
        ...
        this._htOption = {
            ...
            correctLevel: options.correctLevel || QRErrorCorrectLevel.M
        };
