...
class XTAPOMusicApp {
    constructor() {
        ...
        this.isAutoplaySimilar = localStorage.getItem('xtapo_autoplay_similar') !== 'false';
        this.recentPlayedTrackKeys = new Set();
    ...
    loadTrack(trackIndex, autoPlay = true) {
        ...
        // Record recently played track key
        if (track) {
            const trackKey = track.msgId ? `id:${track.msgId}` : `name:${(track.name || '').toLowerCase()}`;
            this.recentPlayedTrackKeys.add(trackKey);
        }

        // Cập nhật trạng thái nút Song Radio
        if (this.songRadioBtn) {
            this.songRadioBtn.classList.toggle('active', !!album.isSmartRadio);
        }

        // Cập nhật Spotlight Card trong Smart Mix modal
        this.updateSmartSpotlightUI();
    ...
    nextTrack() {
        ...
        if (this.isShuffle) {
            ...
            if (this.currentTrackIndex < album.tracks.length - 1) {
                ...
                } else if (this.isAutoplaySimilar) {
                    this.playNextSimilarAutoplayTrack();
                } else {
    ...
    setupControlEvents() {
        ...
        // Autoplay Infinite Button
        if (this.autoplayBtn) {
            this.autoplayBtn.classList.toggle('active', this.isAutoplaySimilar);
            this.autoplayBtn.addEventListener('click', () => this.toggleAutoplaySimilar());
        }

        // Song Radio Button (Player Bar)
        if (this.songRadioBtn) {
            this.songRadioBtn.addEventListener('click', () => {
                if (this.currentTrack) {
                    this.startSongRadio(this.currentTrack);
                } else {
    ...
    setupModalEvents() {
        ...
        // Smart Mix & Recommendations Modal Triggers
        const openSmartMixHandler = () => {
            this.closeMobileDrawer();
            this.openSmartMixModal();
        };
        if (this.navSmartMix) {
        ...
        if (this.mobileNavSmartMix) {
            this.mobileNavSmartMix.addEventListener('click', (e) => {
                e.preventDefault();
                openSmartMixHandler();
            });
        }
        if (this.closeSmartMixModal) {
    ...
    openModal(modal) {
        ...
        } else if (modal === this.smartMixModal) {
            this.saveActiveView({ type: 'smartmix' });
        }
    }
    ...
    // ==========================================================================
    // SMART PLAYLIST & SONG RECOMMENDATION ENGINE (GỢI Ý & RADIO TƯƠNG ĐỒNG)
    // ==========================================================================
    setupSmartMixEvents() {
        if (this.smartAutoplayCheckbox) {
            this.smartAutoplayCheckbox.checked = this.isAutoplaySimilar;
            this.smartAutoplayCheckbox.addEventListener('change', (e) => {
                this.setAutoplaySimilar(e.target.checked);
            });
        }
        ...
        if (this.smartSpotlightPlayBtn) {
            this.smartSpotlightPlayBtn.addEventListener('click', () => {
                if (this.currentTrack) {
                    ...
                    this.showToast('Vui lòng chọn hoặc phát một bài hát trước!');
                }
            });
        }
    }
    ...
    setAutoplaySimilar(enabled) {
        this.isAutoplaySimilar = !!enabled;
        try {
            localStorage.setItem('xtapo_autoplay_similar', this.isAutoplaySimilar ? 'true' : 'false');
        } catch (e) {}

        if (this.autoplayBtn) {
            this.autoplayBtn.classList.toggle('active', this.isAutoplaySimilar);
            this.autoplayBtn.title = this.isAutoplaySimilar ?
                "Tự động phát bài tương đồng khi hết danh sách (Infinite Autoplay): BẬT" :
                "Tự động phát bài tương đồng khi hết danh sách (Infinite Autoplay): TẮT";
        }
        if (this.smartAutoplayCheckbox) {
            this.smartAutoplayCheckbox.checked = this.isAutoplaySimilar;
        }
    ...
    toggleAutoplaySimilar() {
        this.setAutoplaySimilar(!this.isAutoplaySimilar);
        this.showToast(this.isAutoplaySimilar ? "♾️ Tự động phát thông minh: BẬT" : "♾️ Tự động phát thông minh: TẮT");
    }

    updateSmartSpotlightUI() {
        const track = this.currentTrack;
        const album = this.currentAlbum;
        if (!track) return;

        const trackCover = track.coverUrl || (album && album.coverUrl) || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=1000&auto=format&fit=crop';
        const artist = track.artist || (album && album.artist) || 'XTAPO Artist';
        const country = (track.country && track.country.trim()) || this.detectCountryFromTrack(track) || 'Quốc Tế';
        const genre = (track.genre && track.genre.trim()) || 'Pop';

        if (this.smartSpotlightCover) this.smartSpotlightCover.src = trackCover;
        if (this.smartSpotlightTitle) this.smartSpotlightTitle.textContent = track.name || 'Bài hát đang phát';
        if (this.smartSpotlightArtist) this.smartSpotlightArtist.textContent = `${artist} • ${genre} • ${country}`;
    }
    ...
    getAllLibraryTracks() {
        ...
        this.getBaseAlbums().forEach(album => {
            ...
            (album.tracks || []).forEach(track => {
                ...
                if (!seenKeys.has(key)) {
                    seenKeys.add(key);
                    const country = (track.country && track.country.trim()) || albumCountry || this.detectCountryFromTrack(track) || 'Quốc Tế';
                    const genre = (track.genre && track.genre.trim()) || 'Pop';
                    const coverUrl = track.coverUrl || album.coverUrl || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=1000&auto=format&fit=crop';
                    tracks.push({
                        ...track,
    ...
    detectMoodFromTrack(track) {
        const name = (track.name || '').toLowerCase();
        const artist = (track.artist || '').toLowerCase();
        const genre = (track.genre || '').toLowerCase();
        const raw = `${name} ${artist} ${genre}`;

        const moods = [];

        // Chill & Lofi
        if (/lofi|chill|acoustic|mưa|đêm|ngủ|nhẹ|slow|coffee|thư giãn|ambient|piano|guitar|vũ\.|chillies|ngọt|indie/.test(raw)) {
            moods.push('chill');
        }

        // Deep Ballad & Sầu
        if (/ballad|buồn|nhớ|khóc|chia tay|cô đơn|đau|tình|xa|lệ|trữ tình|bolero|sau tất cả|nước mắt|vết thương/.test(raw)) {
            moods.push('ballad');
        }

        // Energy & Workout EDM
        if (/remix|edm|dance|club|party|house|electro|energy|vinahouse|bass|dj|quẩy|sôi động|trap/.test(raw)) {
            moods.push('energy');
        }

        // Romance & Love
        if (/yêu|thương|hạnh phúc|mơ|nắng|ngọt|love|sweet|em|anh|hôn|cưới|bên nhau/.test(raw)) {
            moods.push('love');
        }

        // Acoustic & Indie
        if (/acoustic|unplugged|guitar|mộc|cover|indie|live session/.test(raw)) {
            moods.push('acoustic');
        }
    ...
    calculateTrackSimilarity(targetTrack, candTrack) {
        if (!targetTrack || !candTrack) return 0;
        const targetKey = targetTrack.msgId ? `id:${targetTrack.msgId}` : `name:${(targetTrack.name || '').toLowerCase()}`;
        const candKey = candTrack.msgId ? `id:${candTrack.msgId}` : `name:${(candTrack.name || '').toLowerCase()}`;

        if (targetKey === candKey) return 0; // Skip identical track

        let score = 0;

        const targetArtist = (targetTrack.artist || '').toLowerCase().trim();
        const candArtist = (candTrack.artist || '').toLowerCase().trim();
        const targetCountry = (targetTrack.country || '').trim();
        const candCountry = (candTrack.country || '').trim();
        const targetGenre = (targetTrack.genre || '').toLowerCase().trim();
        const candGenre = (candTrack.genre || '').toLowerCase().trim();

        // 1. Artist Matching
        if (targetArtist && candArtist) {
            if (targetArtist === candArtist) {
                score += 45;
            } else if (targetArtist.includes(candArtist) || candArtist.includes(targetArtist)) {
                score += 28;
            }
        }

        // 2. Genre Matching
        if (targetGenre && candGenre) {
            if (targetGenre === candGenre) {
                score += 32;
            } else if (
                (targetGenre.includes('pop') && candGenre.includes('pop')) ||
                (targetGenre.includes('ballad') && candGenre.includes('pop')) ||
                (targetGenre.includes('acoustic') && candGenre.includes('chill')) ||
                (targetGenre.includes('edm') && candGenre.includes('dance'))
            ) {
                score += 18;
            }
        }

        // 3. Country / Language Matching
        if (targetCountry && candCountry && targetCountry === candCountry) {
            score += 22;
        }

        // 4. Mood & Vibe Overlap
        const targetMoods = this.detectMoodFromTrack(targetTrack);
        const candMoods = this.detectMoodFromTrack(candTrack);
        const hasSharedMood = targetMoods.some(m => candMoods.includes(m));
        if (hasSharedMood) {
            score += 25;
        }

        // 5. Title word token overlap
        const targetWords = (targetTrack.name || '').toLowerCase().split(/\s+/).filter(w => w.length > 2);
        const candWords = (candTrack.name || '').toLowerCase().split(/\s+/).filter(w => w.length > 2);
        const wordOverlap = targetWords.filter(w => candWords.includes(w)).length;
        score += Math.min(15, wordOverlap * 5);

        // 6. Hi-Res format quality bonus
        if (candTrack.format && /flac|24-bit|lossless|dxd|dsd/i.test(candTrack.format)) {
            score += 4;
        }

        // 7. Subtle randomness for organic variety (+0 to +6)
        score += Math.floor(Math.random() * 7);

        return score;
    }

    startSongRadio(seedTrack = null) {
        const target = seedTrack || this.currentTrack;
        if (!target) {
            this.showToast('Chưa có bài hát nào được chọn để tạo Radio.');
            return;
        }

        const allTracks = this.getAllLibraryTracks();
        if (allTracks.length === 0) {
            this.showToast('Thư viện nhạc chưa có đủ bài hát.');
            return;
        }

        // Calculate similarity scores for all candidate tracks
        const scoredCandidates = [];
        allTracks.forEach(cand => {
            const score = this.calculateTrackSimilarity(target, cand);
            if (score > 0) {
        ...
        // Artist Diversity Balancing (Max 3 songs per artist)
        const radioTracks = [target];
        const artistCounts = new Map();
        const targetArtistLow = (target.artist || '').toLowerCase();
        artistCounts.set(targetArtistLow, 1);

        for (const item of scoredCandidates) {
            if (radioTracks.length >= 30) break;
            const a = (item.track.artist || '').toLowerCase();
            const count = artistCounts.get(a) || 0;
            if (count < 3) {
                artistCounts.set(a, count + 1);
                radioTracks.push(item.track);
            }
        }

        // If not enough diverse tracks, fill up with top scoring
        if (radioTracks.length < 15) {
            for (const item of scoredCandidates) {
                if (radioTracks.length >= 25) break;
                if (!radioTracks.some(t => (t.msgId && t.msgId === item.track.msgId) || t.name === item.track.name)) {
                    radioTracks.push(item.track);
                }
            }
        }
        ...
        const radioAlbum = {
            id: `radio-${target.msgId || Date.now()}`,
            isVirtual: true,
            isSmartRadio: true,
            title: `RADIO: ${target.name}`,
            artist: `Luồng nhạc tương đồng "${target.artist || 'Bài Hát'}"`,
            publisher: 'XTAPO Smart Recommendation Engine',
            year: new Date().getFullYear().toString(),
            ...
            glowColors: {
                glow1: 'radial-gradient(circle, #0284c7 0%, #0369a1 60%, transparent 80%)',
                glow2: 'radial-gradient(circle, #9333ea 0%, #7e22ce 60%, transparent 80%)'
            },
            tracks: radioTracks
        };

        this.setVirtualAlbum(radioAlbum, 0, true);
        if (this.songRadioBtn) this.songRadioBtn.classList.add('active');
    ...
    startArtistRadio(artistName) {
        if (!artistName) return;
        const allTracks = this.getAllLibraryTracks();
        const targetLow = artistName.toLowerCase().trim();

        // 1. Tracks by this artist
        const artistTracks = allTracks.filter(t => (t.artist || '').toLowerCase().includes(targetLow));

        // 2. Similar tracks by related artists in same country/genre
        const otherTracks = allTracks.filter(t => !(t.artist || '').toLowerCase().includes(targetLow));
        const sampleTrack = artistTracks[0] || { artist: artistName, name: artistName, country: 'Việt Nam', genre: 'Pop' };

        const scoredOthers = otherTracks.map(cand => ({
            track: cand,
            score: this.calculateTrackSimilarity(sampleTrack, cand)
        })).sort((a, b) => b.score - a.score);

        const radioTracks = [...artistTracks];
        const artistCounts = new Map();
        ...
        if (radioTracks.length === 0) {
            this.showToast(`Chưa có đủ bài hát để tạo Radio cho ${artistName}.`);
            return;
        }

        const radioAlbum = {
            id: `artist-radio-${encodeURIComponent(artistName)}`,
            isVirtual: true,
            isSmartRadio: true,
            title: `ARTIST RADIO: ${artistName}`,
            artist: `Tuyển tập & Nghệ sĩ tương tự ${artistName}`,
            publisher: 'XTAPO Smart Recommendation Engine',
            year: new Date().getFullYear().toString(),
            coverUrl: (radioTracks[0] && radioTracks[0].coverUrl) || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=1000&auto=format&fit=crop',
            tracks: radioTracks
        };

        this.setVirtualAlbum(radioAlbum, 0, true);
        this.showToast(`🎙️ Đang phát Artist Radio: ${artistName} (${radioTracks.length} bài)`);
    }

    generateSmartMixPlaylists() {
        const allTracks = this.getAllLibraryTracks();
        if (allTracks.length === 0) return [];

        const favTracks = this.favoriteTracks || [];
        const favTrackKeys = new Set(favTracks.map(f => f.msgId ? `id:${f.msgId}` : `name:${(f.name || '').toLowerCase()}`));

        // 1. Dành Riêng Cho Bạn (Made For You / Daily Mix)
        const forYouTracks = [];
        const forYouSeen = new Set();
        // Priority to favorites
        allTracks.forEach(t => {
            if (favTrackKeys.has(t._trackKey) && !forYouSeen.has(t._trackKey)) {
                forYouSeen.add(t._trackKey);
                forYouTracks.push(t);
            }
        });
        // Supplement with diverse high similarity tracks
        if (forYouTracks.length > 0) {
            const seed = forYouTracks[0];
            allTracks.forEach(t => {
                if (forYouTracks.length >= 35) return;
                if (!forYouSeen.has(t._trackKey) && this.calculateTrackSimilarity(seed, t) > 35) {
                    forYouSeen.add(t._trackKey);
                    forYouTracks.push(t);
                }
            });
        }
        if (forYouTracks.length < 15) {
            allTracks.forEach(t => {
                if (forYouTracks.length >= 30) return;
                if (!forYouSeen.has(t._trackKey)) {
                    forYouSeen.add(t._trackKey);
                    forYouTracks.push(t);
                }
            });
        ...
        const chillTracks = allTracks.filter(t => {
            const moods = this.detectMoodFromTrack(t);
            return moods.includes('chill') || moods.includes('acoustic');
        });

        // 3. Acoustic Coffee & Mộc Mạc
        const acousticTracks = allTracks.filter(t => {
            const moods = this.detectMoodFromTrack(t);
            return moods.includes('acoustic') || /acoustic|guitar|mộc|vũ\.|chillies|ngọt|trịnh công sơn/.test((t.name + ' ' + t.artist).toLowerCase());
        });

        // 4. Tâm Trạng & Deep Ballad
        const balladTracks = allTracks.filter(t => {
            const moods = this.detectMoodFromTrack(t);
            return moods.includes('ballad') || (t.genre || '').toLowerCase().includes('ballad') || (t.genre || '').toLowerCase().includes('trữ tình');
        });

        // 5. Workout & EDM Remix Energy
        const edmTracks = allTracks.filter(t => {
            const moods = this.detectMoodFromTrack(t);
            return moods.includes('energy') || /remix|edm|dance|house|vinahouse|dj/.test((t.name + ' ' + t.genre).toLowerCase());
        });

        // 6. V-Pop Tuyển Chọn Cực Phẩm
        const vpopTracks = allTracks.filter(t => (t.country || '') === 'Việt Nam');

        // 7. US-UK & Global Top Hits
        const usukTracks = allTracks.filter(t => (t.country || '') === 'Âu Mỹ' || (t.country || '') === 'Quốc Tế');

        // 8. Giai Điệu Bất Hủ (Nostalgia Classics)
        const nostalgiaTracks = allTracks.filter(t => {
            const raw = `${t.name} ${t.artist} ${t.albumTitle || ''}`.toLowerCase();
            return /bất hủ|classic|80s|90s|2000|retro|bolero|trịnh công sơn|đan trường|lam trường|queen|beatles|michael jackson|celine dion|westlife/.test(raw);
        });
        ...
        return [
            {
                ...
                desc: 'Tuyển tập đặc biệt tổng hợp từ các bài hát yêu thích và gu nghe nhạc của bạn',
                icon: '🔮',
                badge: 'Daily Mix',
                gradient: 'linear-gradient(135deg, rgba(56, 189, 248, 0.3) 0%, rgba(147, 51, 234, 0.3) 100%)',
                tracks: forYouTracks
            },
            {
                id: 'night_chill',
                title: '🌙 Night Chill & Lofi',
                desc: 'Giai điệu êm dịu, lofi và acoustic thư giãn cho buổi tối nhẹ nhàng',
                icon: '🌙',
                badge: 'Thư Giãn',
                gradient: 'linear-gradient(135deg, rgba(99, 102, 241, 0.3) 0%, rgba(168, 85, 247, 0.3) 100%)',
                tracks: chillTracks.length > 0 ? chillTracks : allTracks.slice(0, 25)
            },
            {
                id: 'acoustic_coffee',
                title: '☕ Acoustic Coffee',
                desc: 'Những bản nhạc mộc mạc, guitar và piano sâu lắng ngập tràn cảm xúc',
                icon: '☕',
                badge: 'Acoustic',
                gradient: 'linear-gradient(135deg, rgba(245, 158, 11, 0.3) 0%, rgba(217, 119, 6, 0.3) 100%)',
                tracks: acousticTracks.length > 0 ? acousticTracks : allTracks.slice(0, 25)
            },
            {
                id: 'deep_ballad',
                title: '💔 Deep Ballad & Sầu Lắng',
                desc: 'Những ca khúc ballad da diết chạm đến trái tim những ngày tâm trạng',
                icon: '💔',
                badge: 'Tâm Trạng',
                gradient: 'linear-gradient(135deg, rgba(239, 68, 68, 0.3) 0%, rgba(185, 28, 28, 0.3) 100%)',
                tracks: balladTracks.length > 0 ? balladTracks : allTracks.slice(0, 25)
            },
            {
                id: 'workout_edm',
                title: '⚡ Workout & EDM Remix',
                desc: 'Nhịp điệu sôi động, bass căng tràn năng lượng bùng nổ cho buổi tập luyện',
                icon: '⚡',
                badge: 'High Energy',
                gradient: 'linear-gradient(135deg, rgba(236, 72, 153, 0.3) 0%, rgba(147, 51, 234, 0.3) 100%)',
                tracks: edmTracks.length > 0 ? edmTracks : allTracks.slice(0, 25)
            },
            {
                id: 'vpop_hits',
                title: '🎙️ V-Pop Cực Phẩm',
                desc: 'Tuyển tập những bản hit nhạc Việt được yêu thích nhất từ kho nhạc của bạn',
                icon: '🇻🇳',
                badge: 'Nhạc Việt',
                gradient: 'linear-gradient(135deg, rgba(239, 68, 68, 0.35) 0%, rgba(245, 158, 11, 0.25) 100%)',
                tracks: vpopTracks.length > 0 ? vpopTracks : allTracks.slice(0, 25)
            },
            {
                id: 'usuk_global',
                title: '🇺🇸 US-UK & Global Hits',
                desc: 'Tuyển tập giai điệu Pop, R&B và Rock đỉnh cao từ các siêu sao quốc tế',
                icon: '🇺🇸',
                badge: 'International',
                gradient: 'linear-gradient(135deg, rgba(2, 132, 199, 0.35) 0%, rgba(59, 130, 246, 0.25) 100%)',
                tracks: usukTracks.length > 0 ? usukTracks : allTracks.slice(0, 25)
            },
            {
                id: 'nostalgia',
                title: '📻 Giai Điệu Bất Hủ',
                desc: 'Những ca khúc vượt thời gian gắn liền với ký ức và thanh xuân',
                icon: '📻',
                badge: 'Classics',
                gradient: 'linear-gradient(135deg, rgba(16, 185, 129, 0.3) 0%, rgba(13, 148, 136, 0.3) 100%)',
                tracks: nostalgiaTracks.length > 0 ? nostalgiaTracks : allTracks.slice(0, 25)
            }
        ];
    }

    startSmartMix(mixId, isShuffle = false) {
        const mixes = this.generateSmartMixPlaylists();
        const targetMix = mixes.find(m => m.id === mixId);
        if (!targetMix || targetMix.tracks.length === 0) {
            this.showToast('Tuyển tập này hiện chưa có bài hát nào!');
            return;
        }

        let tracksToPlay = [...targetMix.tracks];
        if (isShuffle) {
        ...
        const virtualAlbum = {
            ...
            isVirtual: true,
            isSmartRadio: true,
            title: targetMix.title,
            artist: 'XTAPO Smart Mix',
            publisher: 'XTAPO Curated Recommendations',
            year: new Date().getFullYear().toString(),
            coverUrl: (tracksToPlay[0] && tracksToPlay[0].coverUrl) || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=1000&auto=format&fit=crop',
            tracks: tracksToPlay
        };

        this.closeModal(this.smartMixModal);
        this.setVirtualAlbum(virtualAlbum, 0, true);
        if (this.songRadioBtn) this.songRadioBtn.classList.add('active');
        this.showToast(`✨ Đang phát Smart Mix: ${targetMix.title} (${tracksToPlay.length} bài)`);
    }

    async saveSmartMixToUser(mixObj) {
        if (!mixObj || !mixObj.tracks || mixObj.tracks.length === 0) {
            this.showToast('Không có bài hát để lưu thành playlist.');
            return;
        }

        const playlistName = mixObj.title.replace(/^[^\w\s\u00C0-\u1EF9]+/u, '').trim();
        ...
        try {
            ...
            if (this.currentUser) {
                ...
                const data = await res.json();
                if (res.ok && data.status === 'success' && data.playlist) {
                    const plId = data.playlist.id;
                    // Add tracks to this newly created playlist
                    await fetch(`/api/music/user/playlists/${plId}`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ tracks: mixObj.tracks })
                    });
                    await this.loadPlaylists();
                    this.showToast(`✅ Đã lưu Smart Playlist "${playlistName}" vào Thư viện Playlists của bạn!`);
                } else {
                    this.showToast(data.message || 'Lỗi khi tạo playlist trên máy chủ');
                }
            } else {
                // Guest mode: save to local playlist
                const newPl = {
                    id: `local-${Date.now()}`,
                    name: playlistName,
                    tracks: mixObj.tracks
                };
                this.playlists.push(newPl);
                this.showToast(`✅ Đã lưu Smart Playlist "${playlistName}" vào danh sách Playlists (Guest Mode)!`);
            }
        } catch (e) {
            this.showToast('Lỗi khi lưu Smart Playlist.');
        }
    }

    playNextSimilarAutoplayTrack() {
        const current = this.currentTrack;
        const allTracks = this.getAllLibraryTracks();
        if (allTracks.length === 0) return;

        // Filter out recently played tracks in this session
        const unplayedCandidates = allTracks.filter(t => !this.recentPlayedTrackKeys.has(t._trackKey));
        const candidatePool = unplayedCandidates.length >= 5 ? unplayedCandidates : allTracks;

        // Score candidates against current track
        const scored = candidatePool.map(cand => ({
            track: cand,
            score: this.calculateTrackSimilarity(current, cand)
        })).filter(item => item.score > 0);

        scored.sort((a, b) => b.score - a.score);

        const nextChoice = scored[0] ? scored[0].track : candidatePool[Math.floor(Math.random() * candidatePool.length)];
        ...
        // Seamlessly add nextChoice to current album tracks and play
        const album = this.currentAlbum;
        if (album && album.tracks) {
            album.tracks.push(nextChoice);
            this.loadTrack(album.tracks.length - 1, true);
            this.showToast(`♾️ Autoplay: Đang phát bài tương đồng tiếp theo: "${nextChoice.name}"`, 4000);
        } else {
            this.loadTrack(0, true);
        }
    }
    ...
    renderSmartMixModal() {
        ...
        // Render Smart Mix Grid
        if (this.smartMixGrid) {
            this.smartMixGrid.innerHTML = '';
            const smartMixes = this.generateSmartMixPlaylists();

            smartMixes.forEach(mix => {
                const card = document.createElement('div');
                card.className = 'smart-mix-card';
                card.style.background = mix.gradient;

                card.innerHTML = `
                    <div>
                        <div class="smart-mix-card-top">
                            <span class="smart-mix-icon">${mix.icon}</span>
                            <span class="smart-mix-count-badge">${mix.tracks.length} bài • ${mix.badge}</span>
                        </div>
                        <h4 class="smart-mix-title">${this.escapeHtml(mix.title)}</h4>
                        <p class="smart-mix-desc">${this.escapeHtml(mix.desc)}</p>
                    </div>
                    <div class="smart-mix-card-actions">
                        <button class="smart-mix-action-btn btn-play-smart-mix" title="Phát tuyển tập này">
                            <svg viewBox="0 0 24 24" width="13" height="13" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                            <span>Phát</span>
                        </button>
                        <button class="smart-mix-action-btn btn-shuffle-smart-mix" title="Xáo trộn bài hát">
                            <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 3 21 3 21 8"></polyline><line x1="4" y1="20" x2="21" y2="3"></line></svg>
                            <span>Xáo Trộn</span>
                        </button>
                        <button class="smart-mix-action-btn smart-mix-action-save btn-save-smart-mix" title="Lưu thành Playlist của tôi">
                ...
                // Events
                const playBtn = card.querySelector('.btn-play-smart-mix');
                if (playBtn) {
                    playBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        this.startSmartMix(mix.id, false);
                    });
                }

                const shuffleBtn = card.querySelector('.btn-shuffle-smart-mix');
                if (shuffleBtn) {
                    shuffleBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        this.startSmartMix(mix.id, true);
                    });
                }

                const saveBtn = card.querySelector('.btn-save-smart-mix');
                if (saveBtn) {
                    saveBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        this.saveSmartMixToUser(mix);
                    });
                }
        ...
        // Render Similar Artists Chips
        if (this.smartMixArtistChips) {
            ...
            allTracks.forEach(t => {
                ...
                if (a && a !== 'XTAPO Music' && a !== 'Unknown Artist') {
            ...
            topArtists.forEach(([artistName, count]) => {
                const chip = document.createElement('button');
                chip.className = 'smart-artist-chip';
                chip.innerHTML = `
                    <span class="chip-radio-icon">⚡</span>
                    <span>${this.escapeHtml(artistName)}</span>
                    <span style="font-size: 0.7rem; color: var(--text-muted); font-weight: 400;">(${count})</span>
                `;
                chip.addEventListener('click', () => {
                    this.closeModal(this.smartMixModal);
                    this.startArtistRadio(artistName);
                });
